...
function readMetaData(
  ...
  try {
    nowConfig = JSON.parse(yield readFile(resolvePath(path, 'now.json')));
    hasNowJson = true;
  } catch (err) {
  ...
  if (hasNowJson) {
    ...
    if (nowConfig.type) {
      ...
      !(yield exists(resolvePath(path, 'package.json')))
    ) {
  ...
  if (deploymentType === 'npm') {
    if (isStatic) {
      ...
      try {
        pkg = JSON.parse(yield readFile(resolvePath(path, 'package.json')));
      } catch (err) {
    ...
    let docker;
    try {
      const dockerfile = yield readFile(
