...
function run(token) {return __async(function*(){
  ...
  switch (subcommand) {
    ...
    case 'set': {
      if (argv.rules) {
        yield updatePathAlias(alias, argv._[0], argv.rules);
        break;
      }
      if (args.length !== 2) {
        error(
      ...
      yield alias.set(String(args[0]), String(args[1]));
      break;
    }
    default: {
      if (argv._.length === 0) {
        yield reAlias(token, null, help, exit, apiUrl, debug, alias);
        break;
      }
      ...
      if (argv.rules) {
        ...
        yield alias.set(String(argv._[0]), String(argv._[1]));
...
function updatePathAlias(alias, aliasName, rules) {return __async(function*(){
  const start = new Date();
  const res = yield alias.updatePathBasedroutes(String(aliasName), rules);
  const elapsed = ms(new Date() - start);
  if (res.error) {
